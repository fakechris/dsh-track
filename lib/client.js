...
function forceLayout(nodes, edges, w, h) {
	...
	for (const e of edges) {
		...
		if (a !== void 0 && b !== void 0) adj.push([a, b]);
	}
...
function renderGraphSvg(container, view) {
	...
	const circles = view.nodes.map((n) => {
		if (!pos.get(n.id)) return "";
		const color = GV_COLOR[n.kind] ?? "#999";
		const cls = n.kind === "session" || n.kind === "issue" ? "gv-node gv-click" : "gv-node";
		return "<g data-gn=\"" + escapeHtml(n.id) + "\" data-kind=\"" + n.kind + "\"><circle r=\"9\" fill=\"" + color + "\" class=\"" + cls + "\"/><title>" + escapeHtml(n.label) + "</title><text x=\"0\" y=\"20\" text-anchor=\"middle\" class=\"gv-label\">" + escapeHtml(n.label.slice(0, 18)) + "</text></g>";
	}).join("");
	const legend = [
		"session 会话",
		"issue 需求",
		...
		return "<span class=\"gv-legend\"><i style=\"background:" + GV_COLOR[[
			"session",
			"issue",
			"commit",
			"decision"
	...
	container.innerHTML = "<svg class=\"gv-svg\" width=\"100%\" height=\"100%\" viewBox=\"0 0 " + w + " " + h + "\"><rect width=\"100%\" height=\"100%\" fill=\"transparent\"/>" + edgeLines + circles + "</svg><div class=\"gv-legend-bar\">" + legend + "</div><div class=\"gv-hint\">点击 会话/需求 节点跳转到对话</div>";
	container.onclick = (ev) => {
		...
		if (!node) return;
		if (node.kind === "session" && node.sessionId) jumpToConversation({ sessionId: node.sessionId });
		else if (node.kind === "issue" && node.sessionId) jumpToConversation({
