...
import { EVIDENCE_WINDOW_MS, evidenceWeight } from '../lifecycle/state-machine.ts'
import type { TrackStore } from '../store.ts'
import type { CommitArtifact, EvidenceRef, Link } from '../types.ts'
...
export function parseCommitLines(raw: string, repo: string): CommitArtifact[] {
  ...
  if (fields.length % 5 === 1) fields.pop()
  const commits: CommitArtifact[] = []
  for (let i = 0; i + 4 < fields.length; i += 5) {
    const [sha, authorIso, committerIso, subject, body] = fields.slice(i, i + 5) as [string, string, string, string, string]
    const authorAt = Date.parse(authorIso ?? '')
    const committedAt = Date.parse(committerIso ?? '')
    if (!sha || !Number.isFinite(authorAt)) continue
    commits.push({
      id: commitIdFor(sha),
      sha,
...
const LIMITATIONS: Record<NonNullable<Link['evidenceKind']>, string[]> = {
...
export async function scanProjectCommits(
  store: TrackStore,
  cwd: string,
  opts: { dryRun?: boolean; limit?: number; runGit?: GitRunner; now?: number } = {},
  ...
  const linkPayload = (l: Link): string => JSON.stringify([
  ...
  const commitPayload = (c: CommitArtifact): string => JSON.stringify([
    c.sha, c.projectId, c.repo, c.authorAt, c.committedAt, c.subject, c.body ?? null, c.trailers ?? null,
  ])
  const put = async (l: Omit<Link, 'id' | 'createdAt' | 'ingestedAt'>): Promise<void> => {
    count += 1;
    ...
    if (existing !== undefined && linkPayload(existing) === linkPayload(link)) return // unchanged — no publish
  ...
  if (!dryRun) {
    for (const c of commits) {
      ...
      if (existing !== undefined && commitPayload(existing) === commitPayload(c)) continue
  ...
  const explicitPairs = new Set<string>() // "kind|fromId|toId" already linked explicitly
  ...
  for (const g of cwdGraphs) {
    ...
    for (const c of commits) {
      if (explicitPairs.has(`landed-in|${g.sessionId}|${c.id}`)) continue
  ...
  for (const i of issues) {
    ...
    for (const c of commits) {
      if (explicitPairs.has(`implements|${i.id}|${c.id}`)) continue
  ...
  if (!dryRun) {
    ...
    const commitById = new Map<string, CommitArtifact>([...existingCommits, ...commits.map((c) => [c.id, c]) as Array<[string, CommitArtifact]>])
    const signals: Array<{ issueId: string; signal: EvidenceRef }> = []
    for (const l of impl) {
      if (l.kind !== 'implements') continue
      if (l.eventTime === undefined || now - l.eventTime > EVIDENCE_WINDOW_MS) continue
    ...
    if (signals.length > 0) await store.recordIssueEvidenceMany(signals, now)
  }
