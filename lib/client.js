...
function CalendarYarn(props) {
	...
	const dodge = (0, react.useMemo)(() => {
		...
		const off = {};
		for (const [key, ids] of Object.entries(m)) ids.forEach((id, k) => {
			off[key + "|" + id] = (k - (ids.length - 1) / 2) * 16;
		});
		return off;
	}, [data]);
	...
	return (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [(0, react_jsx_runtime.jsx)("div", {
			...
			children: lanes.map((l) => (0, react_jsx_runtime.jsx)("div", {
				style: {
					position: "absolute",
					top: laneY(l.id) - 9,
					right: 8,
			...
			children: (0, react_jsx_runtime.jsxs)("svg", {
				...
				style: { display: "block" },
				children: [
					lanes.map((l, k) => (0, react_jsx_runtime.jsx)("rect", {
						x: 0,
						y: TOPH + k * LANE_H,
						width: W,
						height: LANE_H,
						fill: l.id === "unk" ? "rgba(90,102,116,0.05)" : rgba(l.hue, k % 2 ? .045 : .028)
					}, l.id)),
					Array.from({ length: data.days }, (_, d) => (0, react_jsx_runtime.jsxs)("g", { children: [(0, react_jsx_runtime.jsx)("line", {
						x1: d * DAY_W,
						y1: TOPH,
						x2: d * DAY_W,
						y2: H - 10,
							...
							children: [
								pts.slice(1).map((p, k) => {
									...
									return (0, react_jsx_runtime.jsx)("path", {
										d: "M " + a.x + " " + a.y + " C " + mx + " " + a.y + ", " + mx + " " + p.y + ", " + p.x + " " + p.y,
										fill: "none",
										stroke: hueOf(p.pd.dom),
										strokeWidth: 1.7,
										strokeDasharray: gap ? "3 4" : "none",
										opacity: .8
									}, k);
								}),
								pts.map((p, k) => {
									const r = 3 + Math.sqrt(p.pd.events) / 2.4;
									return (0, react_jsx_runtime.jsxs)("g", { children: [p.pd.multi && (0, react_jsx_runtime.jsx)("circle", {
										cx: p.x,
										cy: p.y,
										r: r + 3,
										fill: "none",
										...
										cy: p.y,
										r,
										fill: rgba(hueOf(p.pd.dom), .9),
										stroke: T.bg,
										strokeWidth: 1,
										children: (0, react_jsx_runtime.jsx)("title", { children: s.id + " · " + dayLabelOf(base, p.pd.day) + " · " + p.pd.events + " events · 主导 " + (p.pd.dom === "unk" ? "未归属" : p.pd.dom) + (p.pd.multi ? " · 当日跨项目" : "") })
									})] }, k);
								}),
								(0, react_jsx_runtime.jsx)("text", {
									x: pts[0].x,
									y: pts[0].y - (3 + Math.sqrt(pts[0].pd.events) / 2.4) - 5,
									textAnchor: "middle",
									fill: dimmed ? T.faint : T.muted,
									fontSize: 8.5,
									fontFamily: T.mono,
									children: s.id.slice(0, 12)
								}),
								(0, react_jsx_runtime.jsx)("line", {
									x1: pts[pts.length - 1].x + 9,
									y1: pts[pts.length - 1].y - 5,
									x2: pts[pts.length - 1].x + 9,
									y2: pts[pts.length - 1].y + 5,
									stroke: T.muted,
									strokeWidth: 1.6
								})
							]
...
function SessionTable(props) {
	...
	return (0, react_jsx_runtime.jsxs)("table", {
		...
		children: [(0, react_jsx_runtime.jsx)("thead", { children: (0, react_jsx_runtime.jsxs)("tr", {
			style: { color: T.faint },
			children: [
			...
			return (0, react_jsx_runtime.jsxs)("tr", {
				...
				style: {
					borderTop: "1px solid " + T.line,
					cursor: "pointer",
					background: selId === s.id ? rgba(GOLD, .06) : "transparent"
				},
				children: [
					...
					(0, react_jsx_runtime.jsx)("td", {
						...
						children: first && (0, react_jsx_runtime.jsx)("button", {
							onClick: (e) => {
								e.stopPropagation();
								onJump({
									sessionId: s.id,
							...
							children: first.req.length > 26 ? first.req.slice(0, 26) + "…" : first.req
						})
					}),
					(0, react_jsx_runtime.jsx)("td", {
						style: {
					...
					(0, react_jsx_runtime.jsx)("td", {
						style: {
							textAlign: "right",
							padding: "3px 8px",
							color: s.switches > 0 ? GOLD : T.muted
						},
						children: s.switches
					}),
					(0, react_jsx_runtime.jsx)("td", {
						style: { padding: "3px 8px" },
						children: s.projects.length > 1 ? "缠绕(" + s.projects.length + ")" : s.projects[0] ? s.projects[0].slice(0, 8) : "—"
					})
				]
			}, s.id);
		}) })]
	...
	return (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [(0, react_jsx_runtime.jsxs)("div", {
			...
			children: [(0, react_jsx_runtime.jsx)("div", {
				...
				children: (0, react_jsx_runtime.jsx)(CalendarYarn, {
					data,
					selId,
					setSelId,
					hover,
			...
			children: (0, react_jsx_runtime.jsx)(SessionTable, {
				data,
				selId,
				setSelId,
				onJump
...
function mountCalendar(container, data, onJump) {
	if (calRoot === null) calRoot = (0, react_dom_client.createRoot)(container);
	calRoot.render((0, react_jsx_runtime.jsx)(CalendarYarnRoot, {
		data,
		onJump
	}));
}
...
async function renderGraph() {
	...
	let cal = null;
	try {
		cal = (await fetch("/api/track/calendar?cwd=" + encodeURIComponent(cwd)).then((r) => r.json())).calendar ?? null;
	} catch {
		cal = null;
	}
