...
import type { GraphNode, SessionGraph } from '../types.ts'

const truncate = (s: string, max: number): string => (s.length > max ? s.slice(0, max) + '…' : s)
const shortId = (s: string): string => (s.length > 10 ? s.slice(0, 8) + '…' : s)
const seqLabel = (n: GraphNode): string => (n.citation.seqStart === n.citation.seqEnd
  ? '#' + n.citation.seqStart
 ...
 */
export function renderGraphText(g: SessionGraph): string {
  const nodeById = new Map(g.nodes.map((n) => [n.id, n]))
  const children = new Map<string, string[]>()
  const provokedTurn = new Map<string, string>() // user-message id → turn id
  for (const e of g.edges) {
    if (e.kind === 'provoked') { provokedTurn.set(e.fromId, e.toId); continue }
  ...
  const kidsOf = (id: string): string[] =>
    (children.get(id) ?? []).sort((a, b) =>
      (nodeById.get(a)?.citation.seqStart ?? 0) - (nodeById.get(b)?.citation.seqStart ?? 0)
        || (a < b ? -1 : 1))
  ...
  const render = (id: string, depth: number): void => {
    ...
    switch (n.kind) {
      case 'session': {
        const facts: string[] = [];
        if (g.header.cwd) facts.push(g.header.cwd)
        if (g.header.parentSession) facts.push('forked from ' + g.header.parentSession)
        if (g.header.origin === 'subagent') facts.push('subagent (depth ' + (g.header.delegationDepth ?? 1) + ')')
      ...
      case 'turn': lines.push(pad + '▸ ' + n.title + ' [' + seqLabel(n) + ']'); break
      case 'step': lines.push(pad + '· step ' + (n.step ?? '') + ' [' + seqLabel(n) + ']'); break
      case 'tool': {
    ...
    if (n.kind === 'session') {
      for (const kid of kidsOf(id)) {
        ...
        if (k?.kind === 'user-message' && provokedTurn.has(kid)) continue
        render(kid, depth + 1)
      }
    } else if (n.kind === 'turn') {
      for (const kid of kidsOf(id)) render(kid, depth + 1)
      for (const [uid, tid] of provokedTurn) {
        if (tid === id) render(uid, depth + 1)
      }
    } else {
      for (const kid of kidsOf(id)) render(kid, depth + 1)
    }
  ...
  for (const [uid, tid] of provokedTurn) {
    ...
    if (u === undefined || tid === undefined) continue
    if (!g.nodes.some((n) => n.kind === 'turn' && n.id === tid)) lines.push('  💬 (no turn) ' + truncate(u.title, 64) + ' [' + seqLabel(u) + ']')
  }
...
export function renderGraphSummary(g: SessionGraph): string {
