...
function findRowByKey(scroll, key) {
    for (const el of scroll.querySelectorAll('[data-chat-flow-key]')) {
        if (el.dataset.chatFlowKey === key)
            return el;
    }
    return null;
}
...
async function jumpToConversation(opts) {
    ...
    if (key !== undefined) {
        for (let i = 0; i < MAX_PAGES; i++) {
            ...
            if (snap.chat.nodes.get(key) !== undefined)
                break;
            if (!snap.hasMore)
                break;
...
function renderPager(page, totalPages, kind) {
    ...
    if (totalPages <= 1)
        return '';
    return (`<button class="${cls}" ${d}="0" ${page === 0 ? 'disabled' : ''} title="第一页">«</button>` +
...
function render(snapshot) {
    ...
    if (pendingEl !== null) {
        ...
        if (pendingCount !== null)
            pendingCount.textContent = pending.length > 0 ? `(${pending.length})` : '';
    ...
    if (capEl !== null) {
        ...
        if (capturePage >= totalPages)
            capturePage = totalPages - 1;
    ...
    if (issEl !== null) {
        if (issCount !== null)
            issCount.textContent = snapshot.issues.length > 0 ? `(${snapshot.issues.length})` : '';
        ...
        if (batchBar !== null) {
            ...
            for (const id of [...batchSelected]) {
                ...
                if (issue === undefined || issue.state === 'done' || issue.state === 'canceled')
                    batchSelected.delete(id);
            }
            if (batchCount !== null)
                batchCount.textContent = String(batchSelected.size);
...
function renderIssueCard(i) {
    ...
    return (`<div class="inv-card inv-issue-card${expanded ? ' inv-expanded' : ''}" data-id="${i.id}">` +
...
function renderLineageHtml(view, issueId) {
    const parts = [];
    const originLabel = { user_explicit: '用户原话', user_confirmed: '用户确认', agent_proposed: 'agent 提议', system_inferred: '系统推断' };
    ...
    if (parts.length === 1)
        parts.push('<div class="inv-empty">暂无谱系数据</div>');
    return parts.join('');
}
const CAPTURES_PER_PAGE = 8;
const ISSUES_PER_PAGE = 8;
const PRIORITY_LABEL = { 0: 'urgent', 1: 'high', 2: 'medium', 3: 'low', 4: 'none' };
/** Issue group order: active work first, then backlog, then finished. */
const STATE_ORDER = { in_progress: 0, todo: 1, done: 2, canceled: 3 };
/** Issue group display labels (zh, matching the panel language). */
const STATE_LABEL = { in_progress: '进行中', todo: '待办', done: '已完成', canceled: '已取消' };
let capturePage = 0;
let issuePage = 0;
...
const gSeq = (n) => (n.citation.seqStart === n.citation.seqEnd
    ? '#' + n.citation.seqStart
    : '#' + n.citation.seqStart + '-' + n.citation.seqEnd);
/** One tool/step/user row of the graph tree. */
function gRow(kind, inner, extra = '', cls = '') {
...
function renderGraphHtml(doc, sessionId) {
    const nodeById = new Map(doc.nodes.map((n) => [n.id, n]));
    const children = new Map();
    const provoked = new Map();
    for (const e of doc.edges) {
        if (e.kind === 'provoked') {
            provoked.set(e.fromId, e.toId);
            continue;
        }
    ...
    const kidsOf = (id) => (children.get(id) ?? []).sort((a, b) => (nodeById.get(a)?.citation.seqStart ?? 0) - (nodeById.get(b)?.citation.seqStart ?? 0) || (a < b ? -1 : 1));
    const userJump = (n) => (n.messageId
    ...
    const render = (id, depth) => {
        ...
        if (n.kind === 'session') {
            const facts = [];
            if (doc.header.cwd)
                facts.push(escapeHtml(doc.header.cwd));
            if (doc.header.parentSession)
                facts.push('forked from <button class="inv-act inv-jump inv-gfork" data-jump-session="' + escapeHtml(doc.header.parentSession) + '" title="打开父会话">' + escapeHtml(doc.header.parentSession) + '</button>');
            if (doc.header.origin === 'subagent')
                facts.push('subagent (depth ' + (doc.header.delegationDepth ?? 1) + ')');
        ...
        if (n.kind === 'session') {
            for (const kid of kidsOf(id)) {
                ...
                if (k?.kind === 'user-message' && provoked.has(kid))
                    continue;
                render(kid, depth + 1);
            }
        }
...
/** Fetch + render the current session's graph into the panel section. */
async function renderGraph() {
    if (panel === null)
        return;
    const el = panel.querySelector('.inv-graph');
    const title = panel.querySelector('.inv-graph-session');
    if (el === null)
        return;
    const sessionId = activeSessionId();
    if (sessionId === undefined || sessionId === '') {
        el.innerHTML = '<div class="inv-empty">无当前会话 — 打开一个会话后自动显示其执行树</div>';
        if (title !== null)
            title.textContent = '';
        return;
    }
    if (title !== null)
        title.textContent = sessionId.length > 26 ? sessionId.slice(0, 24) + '…' : sessionId;
    let doc = null;
    try {
    ...
    if (doc === null || doc.nodes.length === 0) {
        el.innerHTML = '<div class="inv-empty">尚未构建 — 点「构建」生成当前会话的执行树（或「全构建」批量生成整个工作区）</div>';
        return;
    }
    let relatedHtml = '';
    try {
        const rel = await fetch('/api/track/graph/related?sessionId=' + encodeURIComponent(sessionId)).then((r) => r.json());
        const relData = rel.related ?? {};
        const rows = [];
        if (relData.parent) {
            rows.push('<div class="inv-lg-row">父会话 <button class="inv-act inv-jump" data-jump-session="' + escapeHtml(relData.parent.sessionId) + '">' + escapeHtml((relData.parent.title ?? relData.parent.sessionId).slice(0, 50)) + '</button></div>');
        }
        for (const c of relData.children ?? []) {
            rows.push('<div class="inv-lg-row">子会话 <button class="inv-act inv-jump" data-jump-session="' + escapeHtml(c.sessionId) + '">' + escapeHtml((c.title ?? c.sessionId).slice(0, 50)) + '</button></div>');
        }
        if (rows.length > 0) {
            relatedHtml = '<div class="inv-lg-group">相关会话（多会话关系）</div>' + rows.join('');
        }
    }
    catch {
        relatedHtml = '';
    }
    el.innerHTML = renderGraphHtml(doc, sessionId) + (relatedHtml ? '<div class="inv-related">' + relatedHtml + '</div>' : '');
}
...
function restoreLayout() {
    ...
    if (host.header !== null)
        host.header.classList.remove('inv-host-header');
    if (host.headerWrapper !== null)
        host.headerWrapper.classList.remove('inv-host-header');
    if (host.scrollBody !== null)
        host.scrollBody.classList.remove('inv-host-scroll');
    if (prior.display !== undefined)
        host.candidate.style.display = prior.display;
    if (prior.gridColumns !== undefined)
        host.candidate.style.gridTemplateColumns = prior.gridColumns;
    if (prior.gridRows !== undefined)
        host.candidate.style.gridTemplateRows = prior.gridRows;
}
let graphMode = false;
let graphTab = null;
/** Full-width graph view: hide the sidebar body, take the whole conversation
 *  area (same size as the Chat Trajectory tab). */
function setGraphMode(open) {
    graphMode = open;
    const body = panel?.querySelector('.inv-body');
    const gv = panel?.querySelector('.inv-graphview');
    if (body !== null && body !== undefined)
        body.hidden = open;
    if (gv !== null && gv !== undefined)
        gv.hidden = !open;
    if (host !== null) {
        host.candidate.style.gridTemplateColumns = open
            ? 'minmax(0, 1fr)'
            : (panelWidthPx !== null ? 'minmax(0, 1fr) ' + panelWidthPx + 'px' : 'minmax(0, 2fr) minmax(0, 1fr)');
    }
    if (trackTab !== null)
        trackTab.setAttribute('aria-selected', String(open ? false : !panelOpen));
    if (graphTab !== null)
        graphTab.setAttribute('aria-selected', String(open));
    if (open)
        void renderGraph();
}
function setPanelOpen(open) {
    ...
    if (!open)
        setGraphMode(false);
    if (trackTab !== null)
        trackTab.setAttribute('aria-selected', String(open && !graphMode ? false : true));
    if (graphTab !== null)
        graphTab.setAttribute('aria-selected', String(open && graphMode));
    try {
...
function attach(candidate, header, tablist) {
    ...
    if (header !== null)
        header.classList.add('inv-host-header');
    ...
    if (headerWrapper !== null)
        headerWrapper.classList.add('inv-host-header');
    if (scrollBody !== null)
        scrollBody.classList.add('inv-host-scroll');
    if (panel !== null) {
...
function mountTab() {
    ...
    if (tl === null || tl === undefined)
        return;
    if (tl.querySelector('.inv-tab') !== null)
        return;
    const reference = tl.querySelector(':scope > button[role="tab"][aria-selected="false"]')
        ?? tl.querySelector(':scope > button[role="tab"]');
    if (reference === null)
        return;
    const makeTab = (text) => {
 ...
 */
function locateHost() {
    const sessionTablist = [...document.querySelectorAll('[role="tablist"]')]
        .find((tl) => [...tl.querySelectorAll(':scope > button[role="tab"]')]
        .some((tab) => {
    ...
    if (tablist !== null) {
        ...
        if (header instanceof HTMLElement && header.parentElement instanceof HTMLElement) {
            ...
            return { candidate: header.parentElement, header, headerWrapper: null, scrollBody: header.parentElement.querySelector('[data-conversation-scroll]'), tablist };
        }
    }
    ...
    if (scroll instanceof HTMLElement && scroll.parentElement instanceof HTMLElement) {
        ...
        return { candidate: root, header, headerWrapper: null, scrollBody: scroll, tablist: root.querySelector('[role="tablist"]') };
    }
    return null;
}
function tryMount() {
    if (host !== null && host.candidate.isConnected) {
        ...
        if (panel !== null && panel.parentElement !== host.candidate)
            host.candidate.append(panel);
        // The session tab strip may render AFTER the first attach (blank session
        ...
        if (liveTablist !== null)
            host.tablist = liveTablist;
        host.candidate.classList.add('inv-host');
        if (host.header !== null)
            host.header.classList.add('inv-host-header');
        if (host.headerWrapper !== null)
            host.headerWrapper.classList.add('inv-host-header');
        if (host.scrollBody !== null)
            host.scrollBody.classList.add('inv-host-scroll');
        mountTab();
        syncGrid();
        return;
    }
...
export function mountRightPanel(ctx) {
    clientCtx = ctx;
    // ---- style ----
    ...
    panel.querySelector('.inv-save-config')?.addEventListener('click', () => {
        ...
        settingsPanel?.querySelectorAll('.inv-set').forEach((el) => {
            ...
            if (el.type === 'number' ? Number.isFinite(v) : v !== '')
                patch[key] = v;
        });
    ...
    const onAction = (e) => {
        ...
        const batchToggle = target.closest('[data-batch-toggle]');
        if (batchToggle !== null) {
            batchMode = !batchMode;
            batchSelected.clear();
            batchConfirmTo = null;
        ...
        const batchDone = target.closest('[data-batch-done]');
        if (batchDone !== null) {
            batchConfirmTo = 'done';
            refresh();
            return;
        }
        const batchCancel = target.closest('[data-batch-cancel]');
        if (batchCancel !== null) {
            batchConfirmTo = 'canceled';
            refresh();
            return;
        }
        const batchCommit = target.closest('[data-batch-commit]');
        if (batchCommit !== null) {
        ...
        const batchClear = target.closest('[data-batch-clear]');
        if (batchClear !== null) {
            batchSelected.clear();
            batchConfirmTo = null;
            refresh();
        ...
        if (issueCancelAsk !== null) {
            ...
            if (id)
                confirmIssueAction = { id, to: 'canceled' };
            refresh();
            return;
        }
        ...
        const graphBuild = target.closest('.inv-graph-build');
        if (graphBuild !== null) {
            void buildCurrentGraph();
            return;
        }
        const graphBuildAll = target.closest('.inv-graph-buildall');
        if (graphBuildAll !== null) {
            void buildAllGraphs();
            return;
        }
            ...
            if (id) {
                ...
                if (box !== null) {
                    ...
                    void fetch('/api/track/lineage?entity=' + encodeURIComponent(id))
                        ...
                        .then((d) => {
                        if (d.view)
                            box.innerHTML = renderLineageHtml(d.view, id);
                        else
                            box.innerHTML = '<div class="inv-empty">暂无谱系数据</div>';
                    })
    ...
    const onFocus = () => { tryMount(); refresh(); void renderGraph(); };
    // Re-render the graph when the active session changes.
    let unsubSessions;
    try {
        const sessions = clientCtx.sessions;
        unsubSessions = sessions.list.subscribe(() => { if (panelOpen)
            void renderGraph(); });
    }
    catch {
