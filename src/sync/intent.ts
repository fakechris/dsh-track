 ...
 */
export async function judgeSpanIntent(
  ctx: Context | undefined,
  opts: { provider: string; model: string; requests: string[]; workspaceContext?: string },
): Promise<IntentVerdict | undefined> {
  const llm = getLlm(ctx)
  if (!llm) return undefined
  const context = opts.workspaceContext
 ...
 */
export async function resolveSpanIntent(
  ctx: Context | undefined,
  span: { leadRequest: string; requests: string[] },
  opts: { provider: string; model: string; workspaceContext?: string },
): Promise<IntentVerdict> {
