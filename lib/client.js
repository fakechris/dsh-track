...
function renderPager(page, totalPages, kind) {
	...
	return `<button class="inv-page" ${d}="0" ${page === 0 ? "disabled" : ""} title="第一页">«</button><button class="inv-page" ${d}="${page - 1}" ${page === 0 ? "disabled" : ""} title="上一页">‹</button><input class="inv-page-input" ${input} value="${page + 1}" inputmode="numeric" title="跳到第几页（回车）"><span class="inv-page-info">/ ${totalPages}</span><button class="inv-page" ${d}="${page + 1}" ${page >= totalPages - 1 ? "disabled" : ""} title="下一页">›</button><button class="inv-page" ${d}="${totalPages - 1}" ${page >= totalPages - 1 ? "disabled" : ""} title="最后一页">»</button>`;
}
...
function mountRightPanel(ctx) {
	...
	const onAction = (e) => {
		...
		if (target.closest("[data-batch-commit]") !== null) {
			...
			batchConfirmTo = null;
			batchSelected.clear();
			if (to !== null && ids.length > 0) Promise.all(ids.map((id) => fetch(`/api/track/issues/${encodeURIComponent(id)}/confirm`, {
				method: "POST",
				headers: { "content-type": "application/json" },
