 ...
 */
import { conversationContextKey } from "./platform.js";
 ...
 */
export async function jumpToConversation(opts) {
...
function renderIssueDetail(i) {
    ...
    if (ev !== null && ev !== undefined) {
        const evLabel = { declared: '声明', observed: '观测', candidate: '启发', unmapped: '未归属' };
        const limits = (ev.limitations ?? []).join(' ');
...
export async function buildCurrentGraph() {
...
export function mountRightPanel(ctx) {
    ...
    const onFocus = () => { tryMount(); refresh(); };
