...
export function boundaries(labels) {
    const out = new Array(labels.length).fill(false);
 ...
 */
export function pk(golden, predicted) {
 ...
 */
export function windowDiff(golden, predicted) {
    const n = Math.min(golden.length, predicted.length);
    if (n < 2)
        return 0;
    const gB = boundaries(golden);
...
export function boundaryF1(golden, predicted) {
    const n = Math.min(golden.length, predicted.length);
    if (n < 2)
...
export function evaluateSession(sessionId, golden, predicted) {
