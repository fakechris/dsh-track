...
import type { CommitArtifact, Decision, Issue, Project } from '../types.ts'
...
export interface LineageEvidence {
  ...
  userMessages: Array<{ messageId?: string; title: string; seqStart: number; seqEnd: number }>
}

export interface LineageView {
  target: LineageNode & { state?: string; origin?: string }
  edges: LineageEdge[]
...
const sessionTitleOf = async (store: TrackStore, sessionId: string): Promise<string | undefined> => {
...
export async function resolveNode(
  store: TrackStore,
  id: string,
  kindHint?: 'issue' | 'decision' | 'capture' | 'commit' | 'project' | 'session' | 'epic',
  ...
  if (kindHint === 'issue' || (!kindHint && (id.startsWith('track_issue_') || /[A-Za-z]{2,}-\d+/.test(id)))) {
    ...
    if (i) return { id: i.id, kind: 'issue', title: i.title, meta: { state: i.state, origin: i.origin, semanticKind: i.semanticKind } }
  }
  if (kindHint === 'decision' || (!kindHint && id.startsWith('track_decision_'))) {
    const d: Decision | undefined = await store.getDecision(id)
    if (d) return { id: d.id, kind: 'decision', title: d.question, meta: { status: d.status } }
  }
  if (kindHint === 'capture' || (!kindHint && id.startsWith('track_capture_'))) {
    const c = await store.getCapture(id)
    if (c) return { id: c.id, kind: 'capture', title: c.content }
  }
  if (kindHint === 'commit' || (!kindHint && id.startsWith('track_commit_'))) {
    const cm: CommitArtifact | undefined = await store.getCommit(id)
    if (cm) return { id: cm.id, kind: 'commit', title: cm.subject, meta: { sha: cm.sha } }
  }
  if (kindHint === 'project' || (!kindHint && id.startsWith('track_project_'))) {
    const p: Project | undefined = await store.getProject(id)
    if (p) return { id: p.id, kind: 'project', title: p.name, meta: { path: p.path, repoUrl: p.repoUrl } }
  }
  if (kindHint === 'epic' || (!kindHint && id.startsWith('track_epic_'))) {
    const epics = await store.listEpics()
    const ep = epics.find((x) => x.id === id)
    if (ep) return { id: ep.id, kind: 'epic', title: ep.name, meta: { status: ep.status } }
  }
  if (kindHint === 'session' || (!kindHint && id.startsWith('session'))) {
    const title = await sessionTitleOf(store, id)
    if (title) return { id, kind: 'session', title }
  }
  return null
}
/**
 ...
 */
export async function buildLineage(store: TrackStore, entity: string): Promise<LineageView | null> {
  const target = await resolveNode(store, entity)
  if (!target) return null
  const links = await store.linksFor(target.id)
  const neighbors: Record<string, LineageNode> = {}
  ...
  const commits: Array<{ id: string; sha: string; subject: string; authorAt: number; evidenceKind?: 'declared' | 'observed' | 'candidate' | 'unmapped'; confidence?: number; limitations?: string[] }> = []
  for (const l of links) {
    if (l.kind != 'implements') continue
    const commitId = l.fromId === target.id ? l.toId : l.fromId
    const cm = await store.getCommit(commitId)
    if (cm) commits.push({ id: cm.id, sha: cm.sha, subject: cm.subject, authorAt: cm.authorAt, evidenceKind: l.evidenceKind ?? 'candidate', confidence: l.confidence, limitations: l.limitations })
  }
