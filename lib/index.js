...
import { TrackStore, makeId } from "./store.js";
import { runSync } from "./sync/run.js";
import { createAutoCapture } from "./capture/observe.js";
import { backfillCaptureContext } from "./capture/backfill.js";
import { latestUserRequest } from "./capture/context.js";
import { setUsageRecorder } from "./sync/llm.js";
import { createUsageRecorder, formatUsageReport, summarizeUsage } from "./usage.js";
import { createLifecycleObserver } from "./lifecycle/observe.js";
import { evidenceWeight } from "./lifecycle/state-machine.js";
import { ensureSessionGraph, buildWorkspaceGraphs } from "./graph/service.js";
import { renderGraphSummary, renderGraphText } from "./graph/render.js";
import { writeSemanticLinks } from "./graph/links.js";
import { induceProjects, attributeIssuesBySpan } from "./graph/projects.js";
import { scanProjectCommits } from "./graph/commits.js";
import { buildLineage } from "./graph/lineage.js";
import { relatedSessions, projectGraphView } from "./graph/service.js";
import { buildEvolutionBrief } from "./graph/brief.js";
import { buildCalendar } from "./graph/calendar.js";
...
export function apply(ctx, config) {
     ...
     */
    const recentUser = new Map();
    /** Resolve the current prompt (text + message id) of a session, cached. */
    const promptOf = async (sessionId) => {
        if (!sessionId)
            return undefined;
        const cached = recentUser.get(sessionId);
        if (cached)
            return cached;
        const found = await latestUserRequest(getSessionQuery(ctx), sessionId);
        if (found)
            recentUser.set(sessionId, found);
        return found;
    };
    ...
    const audit = (tool, exec, ok, detail) => {
    ...
    ctx.effect(async () => {
        try {
            ...
            const runScheduledSync = async () => {
                ...
                if (!cfg.syncIntervalDays)
                    return;
                if (Date.now() - lastAutoSync < cfg.syncIntervalDays * 86_400_000)
                    return;
                ...
                for (const ws of workspaces) {
                    try {
                        const report = await runSync({ sessionQuery: sessionQuery, store, ctx }, { cwd: ws, dryRun: false, maxSessions: cfg.syncMaxSessions, engine: cfg.syncEngine });
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Capture an unstructured thought into the Track capture wall for later triage. '
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Raise a decision point that the user must answer before you continue. '
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Record the user\'s answer to a raised decision point. Call this AFTER the user '
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'List decision points from the Track store, optionally filtered by state, time, '
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Declare that the current session is driving a specific issue. Call this when '
        ...
        async execute(args, exec) {
            ...
            return (`Attached ${issue.identifier} (${issue.title}) to this session.\n`
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Propose an issue state change (recorded as model evidence) or confirm one. '
        ...
        async execute(args, exec) {
            ...
            if ((target === 'done' || target === 'canceled') && !args.confirmed_by_user) {
                return (`${issue.identifier} is ${issue.state}; marking it ${target} requires explicit `
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Read the evidence ledger and machine-inferred state of one issue: which signals '
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Create a structured issue (Linear-compatible) in the Track store. '
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'List issues from the Track store, optionally filtered by team and state. '
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Review past user sessions in the current workspace and sync epic/issue-level task history into the Track store. '
        ...
        async execute(args, exec) {
            ...
            const report = await runSync({ sessionQuery: sessionQuery, store, ctx }, options);
    ...
    ctx.tools.register(defineTool({
        name: 'track_usage',
        description: 'Report LLM token usage and estimated cost accumulated by the track engine (v2 sync semantic '
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Backfill motivation context on legacy open captures. Captures created before '
        ...
        async execute(_args, exec) {
            ...
            return (`Capture context backfill — ${result.scanned} legacy open capture(s) inspected.\n`
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Build and read the execution graph (会话结构图) of a session: the deterministic '
        ...
        async execute(args, exec) {
            ...
            await ensureStoreOpen();
            const deps = { sessionQuery: sessionQuery, store };
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Build the genealogy semantic layer: ensure session graphs (with workspace=), write semantic '
        ...
        async execute(args, exec) {
            if (!exec.agent)
                throw new Error('track_genealogy requires an owning agent session');
            await ensureStoreOpen();
            const dryRun = args.dry_run ?? true;
            const sessionQuery = getSessionQuery(ctx);
            const deps = sessionQuery ? { sessionQuery: sessionQuery, store } : undefined;
            let graphs = '';
            if (args.workspace !== undefined) {
                if (!deps)
                    throw new Error('track_genealogy with workspace= requires the session-query service (web profile)');
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Scan a repository\'s git commits into the Track store and align them with the genealogy graph: '
        ...
        async execute(args, exec) {
            if (!exec.agent)
                throw new Error('track_git_artifacts requires an owning agent session');
            await ensureStoreOpen();
            const dryRun = args.dry_run ?? true;
            ...
            if (args.project_level === true) {
                ...
                if (!cwd)
                    throw new Error('track_git_artifacts needs workspace= or a session cwd');
                targets.push(cwd);
            }
            const lines = [`Git artifact scan — ${dryRun ? 'DRY RUN (no writes)' : 'WRITTEN'}.`];
            let totalCommits = 0, totalSessions = 0, totalIssues = 0;
            for (const cwd of targets) {
                const result = await scanProjectCommits(store, cwd, { dryRun, limit });
                if (result.error) {
                    lines.push(`  ! ${cwd}: ${result.error.slice(0, 120)}`);
                    continue;
                }
                totalCommits += result.commits;
                totalSessions += result.sessionsLinked;
                totalIssues += result.issuesLinked;
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Generate the deterministic Evolution Brief for a project (or the whole store): current issue '
        ...
        async execute(args, exec) {
            if (!exec.agent)
                throw new Error('track_evolution_brief requires an owning agent session');
            await ensureStoreOpen();
            ...
            const lines = [];
            if (brief.project)
                lines.push('项目: ' + brief.project.name + ' (' + brief.project.path + ')' + (brief.project.repoUrl ? ' · ' + brief.project.repoUrl : ''));
            const states = Object.entries(brief.issues.byState).map(([k, v]) => k + '=' + v).join(' ') || 'none';
            lines.push('Issue 统计: ' + brief.issues.total + ' 条 [' + states + ']');
            if (Object.keys(brief.issues.bySemantic).length > 0)
                lines.push('语义分布: ' + Object.entries(brief.issues.bySemantic).map(([k, v]) => k + '=' + v).join(' '));
            if (brief.recentCommits.length > 0)
                lines.push('最近 commit: ' + brief.recentCommits.map((c) => c.sha.slice(0, 8)).join(', '));
            if (brief.openDecisions.length > 0)
                lines.push('待确认决策 (' + brief.openDecisions.length + '): ' + brief.openDecisions.map((d) => d.question.slice(0, 40)).join(' | '));
            if (brief.superseded.length > 0)
                lines.push('supersede 链 (' + brief.superseded.length + '): ' + brief.superseded.map((s) => s.newer + ' -> ' + s.older.slice(0, 16)).join(' | '));
            lines.push('');
    ...
    ctx.effect(() => {
        ...
        const seedContext = (sessionId) => latestUserRequest(getSessionQuery(ctx), sessionId);
    ...
    ctx.inject(['webServer'], (serverCtx) => {
        ...
        const registerRoute = (path, handler) => serverCtx.webServer.register({
            kind: 'exact',
            path: `/api/track${path}`,
        ...
        registerRoute('/captures', async (req, res) => {
            ...
            if (req.method === 'POST') {
                ...
                const result = await store.createCapture(capture);
                if (result.status === 'duplicate') {
                    json(res, { ok: true, duplicate: true, capture: result.existing ?? capture });
                    return;
                }
                json(res, { ok: true, capture });
                return;
            }
            if (req.method === 'GET') {
                json(res, { captures: await store.listCaptures() });
                return;
            }
            ...
            if (!includeDeleted) {
                ...
                for (const l of links) {
                    ...
                    if (conf > cur.confidence) {
                        cur.confidence = conf;
                        cur.best = l.evidenceKind ?? 'candidate';
                        cur.limitations = l.limitations;
                    }
        ...
        registerRoute('/decisions', async (req, res) => {
            ...
            if (state !== null && !['pending', 'answered', 'dismissed'].includes(state)) {
                json(res, { error: `invalid state: ${state}` }, 400);
                return;
            }
        ...
        registerRoute('/config', async (req, res) => {
            await ensureStoreOpen();
            if (req.method === 'GET') {
                json(res, { config: await store.readConfig() });
                return;
            }
            if (req.method === 'POST') {
                const body = await readBody(req);
                const patch = {};
                const num = (k) => typeof body[k] === 'number' && Number.isFinite(body[k]) ? body[k] : undefined;
                const d = num('autoCancelPendingDays');
                const i = num('syncIntervalDays');
                const m = num('syncMaxSessions');
                const t = num('nearDupThreshold');
                if (d !== undefined && d >= 0)
                    patch.autoCancelPendingDays = d;
                if (i !== undefined && i >= 0)
                    patch.syncIntervalDays = i;
                if (m !== undefined && m >= 1)
                    patch.syncMaxSessions = m;
                if (t !== undefined && t >= 0 && t <= 1)
                    patch.nearDupThreshold = t;
                if (body.syncEngine === 'v1' || body.syncEngine === 'v2')
                    patch.syncEngine = body.syncEngine;
                if (Object.keys(patch).length === 0) {
                    json(res, { error: 'no valid config fields' }, 400);
                    return;
                }
        ...
        const registerAction = (prefix, handler) => serverCtx.webServer.register({
            kind: 'prefix',
        ...
        registerAction('/captures', async (req, res) => {
            ...
            if (id === null) {
                json(res, { error: 'capture id required' }, 400);
                return;
            }
        ...
        registerAction('/issues', async (req, res) => {
            ...
            if (id === null) {
                json(res, { error: 'issue id required' }, 400);
                return;
            }
            const pathname = new URL(req.url ?? '/', 'http://x').pathname;
            ...
            if (req.method === 'POST' && pathname.endsWith('/confirm')) {
                ...
                if (to !== 'done' && to !== 'canceled') {
                    json(res, { error: 'to must be "done" or "canceled"' }, 400);
                    return;
                }
                ...
                json(res, { ok: true, issue: { id: updated?.id ?? found.id, identifier: updated?.identifier ?? found.identifier, state: updated?.state ?? found.state } });
                return;
            }
            ...
            if (req.method === 'POST' && pathname.endsWith('/dismiss')) {
                ...
                json(res, { ok: true, issue: { id: found.id, identifier: found.identifier, state: found.state } });
                return;
            }
            ...
            if (req.method === 'POST' && pathname.endsWith('/merge')) {
                ...
                if (!into) {
                    json(res, { error: 'into (canonical issue id/identifier) required' }, 400);
                    return;
                }
                ...
                json(res, { ok: true, canonical: { id: merged?.id ?? canonical.id, identifier: merged?.identifier ?? canonical.identifier, state: merged?.state ?? canonical.state } });
                return;
            }
            if (req.method === 'DELETE') {
                ...
                json(res, { ok: true, deleted: deleted !== undefined });
                return;
            }
            json(res, { error: 'method not allowed' }, 405);
        });
        ...
        registerRoute('/issues/batch', async (req, res) => {
            ...
            const body = await readBody(req);
            const raw = Array.isArray(body.ids) ? body.ids.filter((x) => typeof x === 'string') : [];
            const to = body.to;
            if (raw.length === 0) {
                json(res, { error: 'ids (non-empty array) required' }, 400);
                return;
            }
            if (to !== 'done' && to !== 'canceled') {
                json(res, { error: 'to must be "done" or "canceled"' }, 400);
        ...
        registerRoute('/sync', async (req, res) => {
            ...
            try {
                const report = await runSync({ sessionQuery: sessionQuery, store, ctx }, { cwd: workspace ?? ctx.workspace?.cwd ?? '', since, dryRun, maxSessions, engine: body.engine === 'v2' ? 'v2' : 'v1' });
        ...
        registerRoute('/graph', async (req, res) => {
            ...
            if (req.method === 'GET') {
                ...
                if (!sessionId) {
                    json(res, { error: 'sessionId required' }, 400);
                    return;
                }
            ...
            if (req.method === 'POST') {
                ...
                const graph = await ensureSessionGraph({ sessionQuery: sessionQuery, store }, sessionId, body.rebuild === true);
        ...
        registerRoute('/graph/build-all', async (req, res) => {
            ...
            const result = await buildWorkspaceGraphs({ sessionQuery: sessionQuery, store }, cwd, max);
        ...
        registerRoute('/graph/link-all', async (req, res) => {
            ...
            if (typeof body.cwd === 'string' && body.cwd !== '') {
                ...
                graphs = await buildWorkspaceGraphs({ sessionQuery: sessionQuery, store }, body.cwd, typeof body.max_sessions === 'number' ? body.max_sessions : 200);
            }
                ...
                const cwds = [...new Set(all.map((s) => s.header.cwd).filter((c) => typeof c === 'string' && c !== ''))];
                const totals = { total: 0, built: 0, skipped: 0, failed: 0 };
                for (const cwd of cwds) {
                    const r = await buildWorkspaceGraphs({ sessionQuery: sessionQuery, store }, cwd, typeof body.max_sessions === 'number' ? body.max_sessions : 200);
                    totals.total += r.total;
                    totals.built += r.built;
        ...
        registerRoute('/git/scan', async (req, res) => {
            ...
            if (body.project_level === true) {
                ...
                json(res, { error: 'cwd or project_level required' }, 400);
                return;
            }
            const results = [];
            for (const cwd of targets) {
        ...
        registerRoute('/lineage', async (req, res) => {
            ...
            if (!entity) {
                json(res, { error: 'entity required' }, 400);
                return;
            }
            const view = await buildLineage(store, entity);
            if (!view) {
                json(res, { error: 'entity not found' }, 404);
                return;
            }
...
function formatSyncReport(report, dryRun) {
 ...
 */
export function formatDecisionRaised(decision) {
    return (`Decision recorded: ${decision.id}\n`
