...
/** One goal/change event's data (the harness goal facade's structured signal). */
    ...
    const todos: Array<{ content?: string }> | undefined =
      !signals.todo ? undefined
        ...
        : event.type === 'tool/call' && (event.data as ToolCallData | undefined)?.name === 'todo_write'
          ? (() => {
            try {
              return (JSON.parse((event.data as ToolCallData).arguments ?? '{}') as TodoWriteArgs).todos
            } catch { /* malformed arguments — skip */ }
            return undefined
          })()
          : undefined
    if (todos === undefined || sessionId === undefined || todoSeen.has(sessionId)) return
