...
function renderLineageHtml(view, issueId) {
	...
	if (view.commits.length > 0) {
		...
		for (const c of view.commits) {
			...
			parts.push("<div class=\"inv-lg-row\">" + escapeHtml(c.sha.slice(0, 10)) + " · " + escapeHtml(c.subject.slice(0, 60)) + " · " + new Date(c.authorAt).toLocaleDateString() + "<span class=\"" + evCls + "\" title=\"证据强度：" + ev + "（" + (c.evidenceKind ?? "candidate") + "）\">" + ev + "</span></div>");
		}
	}
