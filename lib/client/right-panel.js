...
import { mountGraphForce } from "./graph-force.js";
...
const GV_COLOR = { session: '#4c8dff', issue: '#1a7f37', commit: '#8a8f98', decision: '#e8853c' };
...
function renderGraphSvg(container, view) {
    ...
    container.onclick = (ev) => {
        const g = ev.target.closest('[data-gn]');
        if (g === null)
            return;
        ...
        if (node.kind === 'session' && node.sessionId)
            void jumpToConversation({ sessionId: node.sessionId });
        else if (node.kind === 'issue' && node.sessionId)
            void jumpToConversation({ sessionId: node.sessionId, messageId: node.messageId });
    };
}
...
async function renderGraph() {
    if (panel === null)
        return;
    const el = document.querySelector('#dsh-track-graphview .inv-graph');
    ...
    mountGraphForce(el, view, (n) => {
        if (n.kind === 'session' && n.sessionId)
            void jumpToConversation({ sessionId: n.sessionId });
        else if (n.kind === 'issue' && n.sessionId)
            void jumpToConversation({ sessionId: n.sessionId, messageId: n.messageId });
    });
}
...
function setGraphMode(open) {
    ...
    if (host !== null) {
        ...
        if (host.scrollBody !== null)
            host.scrollBody.style.display = open ? 'none' : '';
        if (host.header !== null)
            host.header.style.display = open ? 'none' : '';
        if (host.headerWrapper !== null)
            host.headerWrapper.style.display = open ? 'none' : '';
    }
    if (panel !== null)
        panel.hidden = open || !panelOpen;
    if (gv !== null)
        gv.hidden = !open;
    if (graphTab !== null)
...
function mountTab() {
    ...
    tl.addEventListener('click', (ev) => {
        const t = ev.target.closest('button[role="tab"]');
        if (t === null || t.classList.contains('inv-tab'))
            return;
        if (graphMode)
            setGraphMode(false);
    }, true);
