 ...
 * session ids.
 */
export function detectForkCopies(profiles: SessionEventProfile[], threshold = 0.9): Array<[string, string]> {
...
export function forkGroups(pairs: Array<[string, string]>): string[][] {
...
export interface MergeGroup {
  ...
  relations: Array<{ from: string; to: string; kind: WorkRelation }>
}
...
export async function classifyRelation(
  ctx: Context,
  opts: { provider: string; model: string; a: TaskCandidate; b: TaskCandidate },
): Promise<{ relation: WorkRelation; confidence: number; reason: string; decidedBy: 'rule' | 'model' } | undefined> {
  ...
  return {
    relation: relation as WorkRelation,
    confidence,
...
export function ruleRelation(a: TaskCandidate, b: TaskCandidate): { relation: WorkRelation; confidence: number; reason: string } {
  ...
  if (ta && ta === tb) return { relation: 'SAME_TASK', confidence: 0.85, reason: 'normalized-title equality' }
  if (ta.includes(tb) || tb.includes(ta)) return { relation: 'CONTINUATION_OF', confidence: 0.4, reason: 'title containment' }
