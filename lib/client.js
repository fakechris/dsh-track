...
function renderIssueCard(i) {
	...
	const actions = isConfirmingDelete ? `<span class="inv-confirm-hint">确认删除任务？</span><button class="inv-act inv-danger" data-issue-del="${i.id}">确认</button><button class="inv-act" data-issue-cancel="1">取消</button>` : `${jump}${actionButtons}<button class="inv-act" data-issue-del-ask="${i.id}">删除</button>`;
...
function renderGraphHtml(doc, sessionId) {
	...
	const render = (id, depth) => {
		...
		if (n.kind === "session") {
			...
			parts.push("<div class=\"inv-groot\">" + escapeHtml(n.title) + " · " + escapeHtml(sessionId) + "<div class=\"inv-gmeta\">" + facts.join(" · ") + " · " + doc.nodes.length + " nodes · " + doc.edges.length + " edges · built " + new Date(doc.builtAt).toLocaleTimeString() + "</div></div>");
