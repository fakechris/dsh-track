...
async function jumpToConversation(opts) {
	...
	const key = messageId !== void 0 && messageId !== "" ? conversationContextKey("input-message", messageId) : void 0;
...
//#endregion
//#region lib/client/deep-link.js
/**
...
*/
async function waitForSessionListed(sessions, sessionId, timeoutMs = LIST_WAIT_MS) {
	const start = Date.now();
	while (Date.now() - start < timeoutMs) {
		const list = sessions.list.getSnapshot();
		if (list.ids.includes(sessionId) || list.byId[sessionId] !== void 0) return true;
		await sleep(POLL_MS);
	}
	return false;
}
/** Remove `?open=` / `?message=` from the URL (one-shot jump), keeping other params. */
function stripQueryParams(location) {
	const params = new URLSearchParams(location.search);
...
async function jumpOnce(sessions, target) {
	if (!await waitForSessionListed(sessions, target.sessionId)) return;
	await jumpToConversation({
...
function GraphView(props) {
	...
	const [building, setBuilding] = (0, react.useState)(null);
	(0, react.useEffect)(() => {
		let alive = true;
		const load = () => {
			setLoading(true);
	...
	const run = (kind) => {
		setBuilding(kind);
		(kind === "cur" ? buildCurrentGraph() : buildAllWorkspaces()).finally(() => setBuilding(null));
	};
	const body = (0, react.useMemo)(() => {
		...
		if (cal === null || cal.sessions.length === 0) return (0, react_jsx_runtime.jsx)("div", {
			...
			children: (0, react_jsx_runtime.jsx)("div", { children: "暂无日历数据 — 点击上方「构建全部会话」生成会话图（或先「构建当前会话」）" })
		});
		...
		children: [(0, react_jsx_runtime.jsxs)("div", {
			...
			children: [
				...
				(0, react_jsx_runtime.jsx)("button", {
					...
					onClick: () => run("cur"),
					children: building === "cur" ? "构建中…" : "构建当前会话"
				}),
				(0, react_jsx_runtime.jsx)("button", {
					style: toolBtn,
					disabled: building !== null,
					onClick: () => run("all"),
					children: building === "all" ? "构建中…" : "构建全部会话"
				}),
				(0, react_jsx_runtime.jsx)("span", {
					style: {
						fontSize: 10.5,
