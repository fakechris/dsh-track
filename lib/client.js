					...
					[...threads.entries()].map(([sid, reqs]) => {
						...
						return (0, react_jsx_runtime.jsx)("path", {
							d: reqs.map((r) => ({
								x: px(r),
								y: py(r)
							})).map((p, k) => k === 0 ? "M " + p.x + " " + p.y : " L " + p.x + " " + p.y).join(" "),
							fill: "none",
							stroke: T.line,
							strokeWidth: 1.2,
							strokeDasharray: "2 3",
...
function CalendarYarnRoot(props) {
	...
	const list = (0, react.useMemo)(() => data.sessions.filter((s) => (activeP.size === 0 || s.projects.some((p) => activeP.has(p))) && (!onlyTangled || s.projects.length > 1) && (!onlyMultiDay || s.activeDays.length > 1)), [
		data,
		activeP,
		onlyTangled,
		onlyMultiDay
