...
export async function writeSemanticLinks(store, dryRun = false) {
    ...
    for (const i of issues) {
        ...
        if (i.parentId)
            await put('issue', i.id, 'issue', i.parentId, 'derives', Date.parse(i.createdAt) || undefined);
        if (i.supersedesIssueId)
            await put('issue', i.id, 'issue', i.supersedesIssueId, 'supersedes', Date.parse(i.createdAt) || undefined);
    }
    for (const c of captures) {
        if (c.promotedToIssueId)
            await put('capture', c.id, 'issue', c.promotedToIssueId, 'derives', Date.parse(c.createdAt) || undefined);
    }
    for (const d of decisions) {
        if (d.sessionId)
            await put('decision', d.id, 'session', d.sessionId, 'raised-in', Date.parse(d.createdAt) || undefined);
        if (d.supersedesDecisionId)
            await put('decision', d.id, 'decision', d.supersedesDecisionId, 'supersedes', Date.parse(d.answeredAt ?? d.createdAt) || undefined);
    }
