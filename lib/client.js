...
function renderLineageHtml(view, issueId) {
	...
	if (view.commits.length > 0) {
		...
		for (const c of view.commits) parts.push("<div class=\"inv-lg-row\">" + escapeHtml(c.sha.slice(0, 10)) + " · " + escapeHtml(c.subject.slice(0, 60)) + " · " + new Date(c.authorAt).toLocaleDateString() + "</div>");
	}
...
const gSeq = (n) => n.citation.seqStart === n.citation.seqEnd ? "#" + n.citation.seqStart : "#" + n.citation.seqStart + "-" + n.citation.seqEnd;
...
function renderGraphHtml(doc, sessionId) {
	...
	const provoked = /* @__PURE__ */ new Map();
	...
	const userJump = (n) => n.messageId ? "<button class=\"inv-act inv-jump\" data-jump-session=\"" + escapeHtml(sessionId) + "\" data-jump-message=\"" + escapeHtml(n.messageId) + "\" title=\"跳回这条 prompt\">↩</button>" : "";
	const parts = [];
	const render = (id, depth) => {
		...
		if (n.kind === "session") {
			...
			parts.push("<div class=\"inv-groot\">" + escapeHtml(n.title) + " · " + escapeHtml(sessionId) + "<button class=\"inv-act inv-jump\" data-jump-session=\"" + escapeHtml(sessionId) + "\" title=\"跳转到这个会话的对话\">↩ 对话</button><div class=\"inv-gmeta\">" + facts.join(" · ") + " · " + doc.nodes.length + " nodes · " + doc.edges.length + " edges · built " + new Date(doc.builtAt).toLocaleTimeString() + "</div></div>");
		} else if (n.kind === "turn") parts.push("<details class=\"inv-gturn\" open><summary>" + pad + escapeHtml(n.title) + "<span class=\"inv-gseq\">" + gSeq(n) + "</span></summary>");
		else if (n.kind === "step") parts.push("<details class=\"inv-gturn\"><summary>" + pad + "· step " + (n.step ?? "") + "<span class=\"inv-gseq\">" + gSeq(n) + "</span></summary>");
		else if (n.kind === "tool") {
			const call = n.callId ? " (" + escapeHtml(n.callId.slice(0, 10)) + "…)" : "";
			parts.push("<div class=\"inv-gtool" + (n.toolError ? " inv-gerr" : "") + "\">" + pad + "⚙ " + escapeHtml(n.toolName ?? n.title) + call + (n.toolError ? " ✗" : " ✓") + "<span class=\"inv-gseq\">" + gSeq(n) + "</span></div>");
		} else if (n.kind === "user-message") parts.push("<div class=\"inv-guser\">" + pad + "<span class=\"inv-guser-text\">💬 " + escapeHtml(n.title) + "</span>" + userJump(n) + "<span class=\"inv-gseq\">" + gSeq(n) + "</span></div>");
		else if (n.kind === "assistant") parts.push(gRow("assistant", pad + "↩ " + escapeHtml(n.title) + "<span class=\"inv-gseq\">" + gSeq(n) + "</span>"));
		if (n.kind === "session") for (const kid of kidsOf(id)) {
			if (nodeById.get(kid)?.kind === "user-message" && provoked.has(kid)) continue;
			render(kid, depth + 1);
		}
			...
			parts.push("</details>");
		} else for (const kid of kidsOf(id)) render(kid, depth + 1);
	};
...
async function renderGraph() {
	...
	if (title !== null) title.textContent = sessionId.length > 26 ? sessionId.slice(0, 24) + "…" : sessionId;
	let doc = null;
	try {
		doc = (await fetch("/api/track/graph?sessionId=" + encodeURIComponent(sessionId)).then((r) => r.json())).doc ?? null;
	} catch {
		doc = null;
	}
	...
	let relatedHtml = "";
	try {
		const relData = (await fetch("/api/track/graph/related?sessionId=" + encodeURIComponent(sessionId)).then((r) => r.json())).related ?? {};
		const rows = [];
		if (relData.parent) rows.push("<div class=\"inv-lg-row\">父会话 <button class=\"inv-act inv-jump\" data-jump-session=\"" + escapeHtml(relData.parent.sessionId) + "\">" + escapeHtml((relData.parent.title ?? relData.parent.sessionId).slice(0, 50)) + "</button></div>");
		for (const c of relData.children ?? []) rows.push("<div class=\"inv-lg-row\">子会话 <button class=\"inv-act inv-jump\" data-jump-session=\"" + escapeHtml(c.sessionId) + "\">" + escapeHtml((c.title ?? c.sessionId).slice(0, 50)) + "</button></div>");
		if (rows.length > 0) relatedHtml = "<div class=\"inv-lg-group\">相关会话（多会话关系）</div>" + rows.join("");
	} catch {
		relatedHtml = "";
	}
...
function setGraphMode(open) {
	...
	if (body !== null && body !== void 0) body.hidden = open;
	if (gv !== null && gv !== void 0) gv.hidden = !open;
	if (host !== null) host.candidate.style.gridTemplateColumns = open ? "minmax(0, 1fr)" : panelWidthPx !== null ? "minmax(0, 1fr) " + panelWidthPx + "px" : "minmax(0, 2fr) minmax(0, 1fr)";
	if (trackTab !== null) trackTab.setAttribute("aria-selected", String(open ? false : !panelOpen));
...
function setPanelOpen(open) {
	...
	if (trackTab !== null) trackTab.setAttribute("aria-selected", String(open && !graphMode ? false : true));
...
function mountTab() {
	...
	tab.addEventListener("click", () => {
		if (panelOpen && !graphMode) setPanelOpen(false);
		else {
			setPanelOpen(true);
			setGraphMode(false);
		}
