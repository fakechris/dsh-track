...
function YarnView(props) {
    ...
    return (_jsx("div", { style: { flex: 1, overflow: 'auto', minWidth: 0, position: 'relative' }, children: _jsxs("div", { style: { position: 'relative', width: 104 + W, height: H }, children: [_jsx("div", { style: { position: 'sticky', top: 0, width: 104 + W, height: TOPH, background: T.bg, zIndex: 2 }, children: Array.from({ length: data.days }, (_, d) => (_jsx("span", { style: { position: 'absolute', left: 104 + d * DAY_W + DAY_W / 2, transform: 'translateX(-50%)', top: 9, fontFamily: T.mono, fontSize: 9, color: T.faint }, children: dayLabelOf(base, d) }, d))) }), _jsx("div", { style: { position: 'sticky', left: 0, width: 104, height: H, background: T.bg, borderRight: '1px solid ' + T.line, zIndex: 3 }, children: lanes.map((l) => (_jsx("div", { style: { position: 'absolute', top: laneY(l.id) - 9, right: 10, fontFamily: T.mono, fontSize: 10.5, color: l.hue }, children: l.name.length > 10 ? l.name.slice(0, 10) + '…' : l.name }, l.id))) }), _jsxs("svg", { width: W, height: H, style: { position: 'absolute', left: 104, top: 0, display: 'block' }, children: [lanes.map((l, k) => (_jsx("rect", { x: 0, y: TOPH + k * LANE_H, width: W, height: LANE_H, fill: l.id === 'unk' ? 'rgba(90,102,116,0.05)' : rgba(l.hue, k % 2 ? 0.045 : 0.028) }, l.id))), Array.from({ length: data.days }, (_, d) => (_jsx("g", { children: _jsx("line", { x1: d * DAY_W, y1: TOPH, x2: d * DAY_W, y2: H - 10, stroke: '#1C242F', strokeWidth: 0.7 }) }, d))), [...threads.entries()].map(([sid, reqs]) => {
                            const dimmed = focus !== null && focus !== sid;
                            const pts = reqs.map((r) => ({ x: px(r), y: py(r) }));
                            const last = pts[pts.length - 1];
                            return (_jsxs("g", { opacity: dimmed ? 0.14 : 1, children: [pts.slice(1).map((p, k) => {
                                        const a = pts[k];
                                        const mx = (a.x + p.x) / 2;
                                        const gap = reqs[k + 1].day - reqs[k].day > 1;
                                        return (_jsx("path", { d: 'M ' + a.x + ' ' + a.y + ' C ' + mx + ' ' + a.y + ', ' + mx + ' ' + p.y + ', ' + p.x + ' ' + p.y, fill: 'none', stroke: hueOf(reqs[k + 1].proj), strokeWidth: 1.7, strokeDasharray: gap ? '3 4' : 'none', opacity: 0.8 }, k));
                                    }), _jsx("text", { x: pts[0].x, y: pts[0].y - (4 + Math.sqrt(reqs[0].events) / 2.2) - 5, textAnchor: 'middle', fill: dimmed ? T.faint : T.muted, fontSize: 8.5, fontFamily: T.mono, children: sid.slice(0, 6) }), _jsx("line", { x1: last.x + 9, y1: last.y - 5, x2: last.x + 9, y2: last.y + 5, stroke: T.muted, strokeWidth: 1.6 })] }, 't' + sid));
