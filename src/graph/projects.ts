...
import { nameOfUrl, repoUrlOf as repoUrlOfPath, type RepoRef } from './repos.ts'
...
export function repoProjectIdFor(url: string): string {
...
export function repoUrlOf(cwd: string): string | undefined {
 ...
 */
export async function induceProjects(store: TrackStore, dryRun = false, now = Date.now()): Promise<ProjectInductionResult> {
  const graphs = await store.listGraphs()
  // Map of projectId -> project draft (repo-touch) or cwd fallback.
  const drafts = new Map<string, { id: string; name: string; path: string; repoUrl?: string; sessionIds: Set<string> }>()
  let sessionsMapped = 0
  for (const g of graphs) {
    const repos: RepoRef[] = Array.isArray(g.header.repos) ? g.header.repos : []
    if (repos.length > 0) {
    ...
    if (Array.isArray(g.header.repos)) continue
  ...
  for (const g of graphs) {
    const repos: RepoRef[] = Array.isArray(g.header.repos) ? g.header.repos : []
    if (repos.length > 0) projBySession.set(g.sessionId, repoProjectIdFor(repos[0]!.url))
    else if (!Array.isArray(g.header.repos) && g.header.cwd) projBySession.set(g.sessionId, projectIdFor(g.header.cwd))
  }
  ...
  for (const issue of issues) {
    ...
    if (sid !== undefined && !projBySession.has(sid) && issue.projectId !== undefined) {
      if (dryRun) { issuesAssigned += 1; continue }
      await store.upsertIssue({ ...issue, projectId: undefined, updatedAt: new Date(now).toISOString() })
      issuesAssigned += 1
      continue
    }
    if (projectId === undefined || issue.projectId === projectId) continue
    if (dryRun) { issuesAssigned += 1; continue }
