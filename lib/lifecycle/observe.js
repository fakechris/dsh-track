 ...
 */
import { evidenceWeight } from "./state-machine.js";
...
export function signalForEvent(event, now = Date.now()) {
    ...
    switch (event.type) {
        ...
        case 'tool/result': {
            ...
            if (err)
                return { signal: 'tool-error', at: now, weight: evidenceWeight('tool-error'), pointer: data.name };
            return undefined;
        }
        case 'tool/call': {
...
export function createLifecycleObserver(ctx, deps) {
    ...
    const attach = (sessionId, issueId) => { attached.set(sessionId, issueId); };
    const detach = (sessionId) => { attached.delete(sessionId); };
