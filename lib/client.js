...
function YarnView(props) {
	...
	const reqMap = (0, react.useMemo)(() => new Map(data.requirements.map((r) => [r.id, r])), [data]);
	const px = (r) => r.day * DAY_W + DAY_W / 2 + (cluster.xo[r.id] ?? 0) + r.sessionId.length * 7 % 7 - 3;
	const py = (r) => laneY(r.proj) + (cluster.yo[r.id] ?? 0);
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)("div", {
								...
								children: "节点=需求(大小=该需求工作量,即其触发的会话事件数) · 紫线=子代理继承 · 黄虚线=需求派生 · 金环=跨项目缠绕"
							})
						]
					}),
					(0, react_jsx_runtime.jsxs)("div", {
