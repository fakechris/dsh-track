...
export function createUsageRecorder(store: TrackStore): (record: Omit<LlmUsageRecord, 'id'>) => void {
  return (record) => {
...
export function sumUsage(records: readonly LlmUsageRecord[]): UsageTotals {
  const t: UsageTotals = {
    calls: 0, ok: 0, fail: 0,
 ...
 */
export function estimateCost(provider: string, model: string, totals: UsageTotals): number | null {
  const price = priceFor(provider, model)
  if (!price) return null
  const cacheWritePerM = price.cacheWritePerM ?? price.inputPerM
  return (
    totals.inputTokens * price.inputPerM
    + totals.cacheReadTokens * (price.cacheReadPerM ?? 0)
    + totals.cacheWriteTokens * cacheWritePerM
    + totals.outputTokens * price.outputPerM
  ) / 1_000_000
}
...
export interface UsageSummary {
  ...
  since?: number
  total: UsageTotals & { costUsd: number | null }
...
export function summarizeUsage(records: readonly LlmUsageRecord[], since?: number): UsageSummary {
  const scoped = since === undefined ? records : records.filter((r) => r.at >= since)
  const byModel = new Map<string, ModelUsageLine>()
  const byLabel = new Map<string, UsageTotals>()
  for (const r of scoped) {
    ...
    labelTotals.calls += 1
    if (r.ok) labelTotals.ok += 1
    else labelTotals.fail += 1
  ...
  for (const line of byModel.values()) {
    ...
    if (line.costUsd === null) { unpriced = true; priced = false } else totalCost += line.costUsd
    lines.push(line)
  }
