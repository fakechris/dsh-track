 ...
 */
import { jumpToConversation } from "./right-panel.js";
const POLL_MS = 120;
...
async function waitForSessionListed(sessions, sessionId, timeoutMs = LIST_WAIT_MS) {
    ...
    while (Date.now() - start < timeoutMs) {
        ...
        if (list.ids.includes(sessionId) || list.byId[sessionId] !== undefined)
            return true;
        await sleep(POLL_MS);
    }
    return false;
...
function stripQueryParams(location) {
    const params = new URLSearchParams(location.search);
    if (!params.has('open') && !params.has('message'))
        return;
    params.delete('open');
...
async function jumpOnce(sessions, target) {
    if (!(await waitForSessionListed(sessions, target.sessionId)))
        return;
    await jumpToConversation({ sessionId: target.sessionId, messageId: target.messageId });
    if (target.fromQuery)
        stripQueryParams(window.location);
}
/**
 ...
 */
export function installDeepLink(ctx) {
